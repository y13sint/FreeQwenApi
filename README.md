...
1. Параметр `include_history`: Не реализован для OpenAI-совместимого API. Вместо этого история всегда определяется по `chatId`.
...
1. **Работа с историей:** При использовании формата `messages` прокси извлекает только последнее сообщение пользователя для добавления в историю. Предыдущие сообщения уже хранятся на сервере и определяются по `chatId`.
