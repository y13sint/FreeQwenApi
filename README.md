...
Прокси можно (опционально) закрыть собственным «Bearer»-токеном.
...
> 💡 Если нужна «память» между запросами и вы используете `/api/chat/completions`, просто сохраняйте `chatId` из ответа и передавайте предыдущие сообщения сами — прокси не делает это автоматически.
